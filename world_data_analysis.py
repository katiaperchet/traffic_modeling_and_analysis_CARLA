...
unique_jammed_ids = df[df['is_stuck'] == 1]['v_id'].nunique()
unique_jam_by_crash_ids = df[df['jam_by_crash'] == 1]['v_id'].nunique()
print("\n" + "="*30)
print(f"INCIDENT SUMMARY")
print(f"="*30)
print(f"Unique vehicles involved in crashes: {unique_crashes}")
print(f"Unique vehicles involved in jamms: {unique_jammed_ids}")
print(f"Unique vehicles involved in jamms by crashes: {unique_jam_by_crash_ids}")
...
jam_evolution = df[df['is_stuck'] == 1].groupby('timestamp')['v_id'].count()
plt.figure(figsize=(10, 5))
jam_evolution.plot(color='red', linewidth=2)
plt.title('Evolution of Traffic Jam (Stuck vehicles over time)')
plt.xlabel('Time (seconds)')
plt.ylabel('Number of vehicles currently jammed')
